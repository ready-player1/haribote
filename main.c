/*
  ...
  This software is a language processor for HL.
...
inline static void saveExpr(int i)
{
  ...
  blockInfo[blockDepth + ForWpc1 - 2 + 2 * i + 1] = wpc[_end(i)];
}
...
inline static void restoreExpr(int i)
{
  ...
  wpc[_end(i)] = blockInfo[blockDepth + ForWpc1 - 2 + 2 * i + 1];
}
...
int compile(String src)
{
  ...
  for (pc = 0; pc < nTokens;) {
    ...
    if (match(0, "!!*0 = !!*1;", pc)) {
      ...
      vars[curBlock[ForBegin]] = icp - internalCode;
    }
    else if (match(12, "}", pc) && curBlock[BlockType] == ForBlock) {
      vars[curBlock[ForContinue]] = icp - internalCode;

      restoreExpr(1);
      ifgoto(1, ConditionIsTrue, curBlock[ForBegin]);
      vars[curBlock[ForBreak]] = icp - internalCode;
  ...
  for (icp = internalCode; icp < end; icp += 5) { // goto先の設定
    ...
    if (OpGoto <= op && op <= OpLop)
      icp[1] = (IntPtr) (internalCode + *icp[1]);
  }
